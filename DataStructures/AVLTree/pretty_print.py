def pretty_print(tree):
    ...
    key_length = 3      # max length of a printed key
    small_space = 3     # space between node siblings on bottom row
    big_space = 5      # space between pairs of siblings on bottom row
    
    # assemble list of lists of all keys, by row
    all_keys = []
    node_row = [tree.root]
    while node_row != [None] * len(node_row):
        key_row = [node.key if node else None for node in node_row]
    ...
    # assemble list of lists of coordinates. These are coordinates where the printed keys will be centered. Bottom row is first calculated using small_space and big_space values; higher rows are adjustd as midpoints
    all_keys.reverse()
    all_coords = []
    pos = key_length//2
    current_coord = []
